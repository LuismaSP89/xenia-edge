  ...
  frame_limiter_worker_thread_ =
    ...
    // Linux uses full sleep duration due to scheduler quantum issues
            ...
            while (frame_limiter_worker_running_) {
              ...
              // vsync=false: Fire vblanks as fast as possible (unlimited)
...
#if XE_PLATFORM_WIN32
              if (vsync_enabled) {
                // Fixed vblank rate mode - use accurate timing (time-gating +
                // 90% sleep + 10% spin)
                ...
                // Fire vblanks as fast as possible for games using delta time
                MarkVblank();
              ...
              if (vsync_enabled) {
                const uint32_t vblank_hz = GetGuestVblankRateHz();
                uint64_t sleep_duration_ns = 1000000000 / vblank_hz;
