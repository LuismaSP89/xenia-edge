  ...
  if (instr.opcode == ucode::FetchOpcode::kGetTextureGradients) {
    ...
    if (is_texture_resolved != spv::NoResult && size_needed_components) {
      if (size[0] != spv::NoResult && draw_resolution_scale_x_ > 1) {
        ...
        size[0] = builder_->createTriOp(spv::OpSelect, type_float_,
                                        is_texture_resolved, scaled_size_x,
                                        size[0]);
      }
      if (size[1] != spv::NoResult && draw_resolution_scale_y_ > 1) {
        ...
        size[1] = builder_->createTriOp(spv::OpSelect, type_float_,
                                        is_texture_resolved, scaled_size_y,
                                        size[1]);
      }
